...
import { PremiumAccessModal } from '@/modules/membership';
...
export function BlogDetailPage() {
  ...
  const getFullVietnameseDate = (dateString?: string) => {
  ...
  const fullFormattedDate = getFullVietnameseDate(blog.publishedAt || blog.createdAt);
  ...
  return (
    <article className="max-w-4xl mx-auto px-4 sm:px-6 py-6 sm:py-10 space-y-6 relative">
      {/* 1. Navigation */}
      <div className="flex items-center justify-between text-xs sm:text-sm text-zinc-500 pb-1">
        <Link
          href="/blogs"
          className="inline-flex items-center gap-1.5 font-semibold text-zinc-600 hover:text-primary transition-colors shrink-0"
        >
      ...
      {/* 2. Clean Header (Date with Weekday only) */}
      <header className="space-y-3 pb-6 border-b border-zinc-200/80">
        {/* Published Date (Full Weekday + Date) */}
        {fullFormattedDate && (
        ...
        {/* Title */}
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-zinc-900 tracking-tight leading-snug sm:leading-tight">
          {blog.title}
        </h1>
      ...
      <main className="pt-2 relative">
        ...
        {/* Author Signature aligned to bottom right */}
        <div className="pt-8 pb-2 flex justify-end">
          <div className="text-right space-y-1">
            <span className="text-xs text-zinc-400 font-medium block uppercase tracking-wider">Tác giả bài viết</span>
            <p className="text-sm sm:text-base font-bold text-zinc-900">{authorName}</p>
          </div>
        </div>
      </main>
      ...
      {/* 7. Back to Top Anchor */}
      {
        showScrollTop && (
          <Button
            ...
            title="Về đầu trang"
            className="fixed bottom-20 right-4 sm:bottom-24 sm:right-6 z-40 !w-11 !h-11 !p-0 flex items-center justify-center cursor-pointer shadow-lg hover:scale-105 transition-all"
          >
