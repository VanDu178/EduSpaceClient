...
export function ServiceCard({ service, className = "" }: ServiceCardProps) {
  ...
  return (
    <div
      className={`bg-white rounded-3xl p-8 flex flex-col justify-between relative transition-all duration-300 hover:-translate-y-1.5 ${
        service.popular
    ...
    >
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          ...
          {service.price && (
            <span className="text-sm font-semibold text-slate-900 px-3 py-1 rounded-full bg-slate-100 border border-slate-200">
              {service.price}
            </span>
          )}
        </div>
