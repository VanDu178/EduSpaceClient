...
export function HlsPlayer({
  ...
  const [currentLevel, setCurrentLevel] = useState<number>(-1); // -1 là Auto (ABR)
    ...
    if (Hls.isSupported()) {
      const hls = new Hls({
        ...
        xhrSetup: (xhr: XMLHttpRequest, url: string) => {
          const isExternalCdn = url.includes('b-cdn.net') || url.includes('bunnycdn') || url.includes('cloudfront');
          if (!isExternalCdn) {
      ...
      hls.on(Hls.Events.MANIFEST_PARSED, (_, data) => {
        ...
        if (autoPlay) {
          videoElement.play().catch(() => {
            // Trình duyệt tự động chặn autoplay nếu chưa tương tác
          });
        }
      });
  ...
  return (
    <div className="relative w-full h-full bg-slate-950 flex items-center justify-center overflow-hidden">
      {/* 1. Màn hình báo lỗi khi Video FAILED hoặc Lỗi Mạng */}
      ...
      {/* 3. Menu Chuyển Đổi Độ Phân Giải (Quality Switcher Gear Menu) */}
      {!isProcessing && !isFailed && !errorMessage && levels.length > 0 && (
        <div className="absolute top-3 right-3 z-20">
          <div className="relative">
            <button
              type="button"
              onClick={() => setIsGearOpen(!isGearOpen)}
              className="flex items-center gap-1.5 bg-slate-900/85 hover:bg-slate-800 backdrop-blur-md text-white text-xs font-semibold px-3 py-1.5 rounded-xl border border-slate-700/80 transition-all cursor-pointer shadow-lg hover:border-sky-500/50"
            ...
            {/* Dropdown Chọn Chất Lượng (360p, 480p, 720p, 1080p, Auto) */}
            {isGearOpen && (
