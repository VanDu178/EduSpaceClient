      ...
      <div className="mb-6">
        <div className="relative max-w-md">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 text-xs sm:text-sm bg-slate-50 border border-slate-200 rounded-xl text-slate-900 placeholder-slate-400 focus:outline-none focus:border-blue-500 transition-colors"
          />
        </div>
      </div>
          ...
          {filteredPosts.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {filteredPosts.map((post) => (
                <div
                  key={post.id}
                  className="bg-slate-50 border border-slate-200 rounded-xl p-5 flex flex-col justify-between hover:border-slate-300 transition-all"
                >
                  <div>
                    <div className="flex items-center justify-between text-xs text-slate-500 mb-2">
                      <span className="px-2.5 py-0.5 rounded-md font-medium bg-blue-100 text-blue-700 border border-blue-200">
                        {post.category}
                      </span>
                  ...
                  <div className="pt-4 border-t border-slate-200 flex items-center justify-between text-xs text-slate-500">
                    <span>Tác giả: <strong className="text-slate-700 font-medium">{post.author}</strong></span>
                    <div className="flex items-center gap-2">
                      ...
                      <Link href={`/posts/${post.slug}`}>
                        <Button
                          ...
                          className="flex items-center text-xs rounded-lg border border-slate-200"
                        >
                          Đọc tiếp
                        </Button>
                      </Link>
      ...
      {activeSubTab === "videos" && (
        <div className="space-y-4">
          {filteredVideos.length === 0 ? (
            <div className="text-center py-12 border border-dashed border-slate-200 rounded-xl">
              <VideoCameraIcon className="w-10 h-10 mx-auto text-slate-300 mb-2" />
              <p className="text-sm font-medium text-slate-600">
              ...
              {filteredVideos.map((video) => (
                ...
                >
                  ...
                  <div className="p-4 flex-1 flex flex-col justify-between">
                    <div>
                      <h3 className="text-base font-semibold text-slate-900 line-clamp-2 mb-2">
                        <Link href={`/videos/${video.slug}`} className="hover:text-blue-600 transition-colors">
                          {video.title}
                        </Link>
                      </h3>
                      <p className="text-xs text-slate-500">
                        Giảng viên: <strong className="text-slate-700 font-medium">{video.instructor}</strong>
                      </p>
                    </div>
