...
export default function PostsPage() {
  ...
  const posts = [
    ...
    },
    {
      ...
      excerpt: "Tối ưu token theme trong Antd ConfigProvider kết hợp với utility classes của Tailwind để tạo giao diện mượt mà không bị conflict CSS.",
    },
    {
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {filteredPosts.map((item) => (
            <Link key={item.slug} href={`/posts/${item.slug}`} className="group">
              <div className="bg-white rounded-3xl overflow-hidden h-full flex flex-col transition-all duration-300 hover:-translate-y-1.5 border border-slate-200 hover:border-blue-500/30">
                <div className="relative h-52 w-full overflow-hidden bg-slate-200">
                  ...
                  <div className="absolute top-3 left-3">
                    <span className="bg-slate-900/70 backdrop-blur-md text-white text-xs px-3 py-1 rounded-full font-medium border border-white/10">
                      {item.category}
                    </span>
                  </div>
                </div>
